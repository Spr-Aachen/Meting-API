...
export const docs = (c) => {
    ...
    <div class="text-lg" style="margin: 20px;padding: 10px;">
        ...
        <div class="panel font-mono text-xl">
            ${new URL('/api', get_url(c)).href}?server=<span class="special-pale">[server]</span>&type=<span
    ...
    const extraScripts = `
        ...
        <script>
            ...
            function sendMusicRequest(mserver,mclass,mid) {
                ...
                const base = "api?server="+mserverv+"&type=";
                if (mclass === 'url') {
                    ap.list.add({ name: mid, artist: mserver, url: base+"url&id="+midv, cover: base+"pic&id="+midv });
                    return;
                }
                if (mclass === 'pic') {
                    window.open(base+"pic&id="+midv, '_blank');
                    return;
                }
                if (mclass === 'lrc') {
                    const xhr = new XMLHttpRequest();
                    xhr.open('GET', base+"lrc&id="+midv);
                    xhr.onload = function() {
                        if (xhr.status >= 200 && xhr.status < 300) {
                            ap.list.add({ name: mid, artist: mserver, url: base+"url&id="+midv, cover: base+"pic&id="+midv, lrc: xhr.responseText });
                        }
                    };
                    xhr.send();
                    return;
                ...
                const xhr = new XMLHttpRequest();
                xhr.open('GET', base+mclassv+"&id="+midv);
                xhr.onload = function() {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        try { ap.list.add(JSON.parse(xhr.responseText)); } catch(e) {}
                    }
                };
                xhr.send();
            }
